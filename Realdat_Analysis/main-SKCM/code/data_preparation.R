########################################################################
## The raw RNAseq, phenotype, and somatic mutation datasets were download from https://xenabrowser.net/datapages/?cohort=GDC%20TCGA%20Melanoma%20(SKCM)&removeHub=https%3A%2F%2Fxena.treehouse.gi.ucsc.edu%3A443
########################################################################
...
pdat = read.csv('../data/raw/TCGA-SKCM.GDC_phenotype.tsv',sep = '\t',header=TRUE,fill=TRUE)[,c('submitter_id.samples','age_at_initial_pathologic_diagnosis')] 
colnames(pdat) = c('sample','age')
## 8 subjects were removed due to missing information. 
...
sdat = read.csv('../data/raw/TCGA-SKCM.muse_snv.tsv',fill=TRUE,header=TRUE,sep='\t',check.names = FALSE)
...
con <- gzcon(url("https://gdc.xenahubs.net/download/TCGA-SKCM.htseq_counts.tsv.gz"))
txt <- readLines(con)
...
ref_gene = c("PDCD1","CD274","PDCD1LG2", "CTLA4","MUC4", "CLEC2D","CXCR4","FCGR2A", "GIT2", "MS4A7","PRDM1","PRDX3", "SDCBP","SPG21","VNN2")
...
## exchange the label such that subtype 3 tends to have the highest expression
ymat[ymat == 1] = 4
ymat[ymat == 3] = 1
ymat[ymat == 4] = 3
...
thres = 3 # mutations with at least 3 mutants were preserved
for (i in 1:npath) {
    ## genotype matrix
    ...
    if(!is.null(G))
        if(length(unique(do.call(rbind,strsplit(colnames(G),'_'))[,1]))<10)
            G = NULL
