...
uMiST <- function(y,X,G,W){ # mist-univariate
    ...
    suppressWarnings({
        ...
        if(is(tryCatch(logit.test(y12,X12,G12,W,"liu"),error=function(e) e, warning=function(w) w),"warning")) p12 = NA
        if(is(tryCatch(logit.test(y13,X13,G13,W,"liu"),error=function(e) e, warning=function(w) w),"warning")) p13 = NA
        if(is(tryCatch(logit.test(y23,X23,G23,W,"liu"),error=function(e) e, warning=function(w) w),"warning")) p23 = NA
        out = c(p12,p13,p23)
    })
    if(sum(is.na(out))==3) res = 1 else res = min(out,na.rm=TRUE)*3
    return(min(res,1))
}
